...
std::vector<pair<uint,uint>> StartEnd (_NODE_COUNT, std::make_pair(_EDGE_COUNT, _EDGE_COUNT));
...
int loadGraph(string sFileName)
{
 ...
 if (inputFile)
 {
  ...
  for (uint i = 0,j = 1; i < _EDGE_COUNT;++j) {
      while(packedVectorEdgesStartingNodes[i] == packedVectorEdgesStartingNodes[j] && j < _EDGE_COUNT){
        ++j;
      }
      // cout << "[" << packedVectorEdgesStartingNodes[i] << "]" << " S: " << i << " E :" << j << endl;
      StartEnd[packedVectorEdgesStartingNodes[i]].first = i;
      StartEnd[packedVectorEdgesStartingNodes[i]].second = j;
      i = j;
}
...
K = strtol(argv[1],NULL,10);
 ...
 // showPackedData();
 Find_WSA(packedVectorEdgesStartingNodes,packedVectorEdges,packedVectorEdgeWeights,StartEnd,K);
 }
 return 0;
}
